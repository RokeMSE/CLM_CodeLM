...
@router.post("/login")
async def login_user_route(res: Response, email: str = Form(...), password: str = Form(...)):
    """
...
async def check_token_route(res: Response, access_token: str = Cookie(None), refresh_token: str = Cookie(None)):
    """
    ...
    try:
        ...
        if not access_token:
            ...
            if not refresh_token:
                ...
                res.set_cookie(key="access_token", value=new_access_token, httponly=True, secure=False, samesite="lax", max_age=ACCESS_TOKEN_EXPIRE_MINUTES*60)
        ...
        if payload.userID is None or get_user_by_id(payload.userID) is None: # check if the userID is existing in the database
            res.status_code = status.HTTP_401_UNAUTHORIZED
            return {"message": "User not found"}
