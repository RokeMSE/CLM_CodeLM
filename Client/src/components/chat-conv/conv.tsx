...
async function getBotResponseFromBackend(userText: string, currentHistory: Message[]): Promise<string> {
    ...
    try {
        const response = await axios.post<BackendSuccessResponse>(backendUrl, requestBody); // Use axios.post
        ...
        if (axios.isAxiosError(error)) {
            ...
            if (error.response) {
                ...
                errorMessage = error.response.data?.detail || error.response.data?.error || `Backend Error: ${error.response.status}`;
...
export default function ChatWindow() {
    ...
    const handleSendMessage = useCallback(async (text: string) => {
        ...
        try {
            ...
            const replyText = await getBotResponseFromBackend(text, historyForBackend); // Pass history
